...
public class MonitoringMemoryClosed2 {
	...
	public static void uploadSolrData() throws IOException {
		...
		String fileName = GavelConstant.FILE_PATH+"MonitoringMemoryClosed_2.xlsx";
		...
		for (Row row : sheet) {
			...
			try {
				for (int cn = 0; cn <= row.getLastCellNum(); cn++) {
					if (row.getRowNum() > 0 && row.getRowNum()<=49) {
					...
					if (row.getRowNum() > 49 && row.getRowNum()<=97) {
		...
		System.out.println(" Closed MonitoringMemory2 loaded successfully");
	}
