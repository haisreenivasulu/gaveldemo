...
public class CPUForecast {
	...
	public static void uploadSolrData() throws IOException {
		...
		deleteSolrData();
		String fileName = GavelConstant.FILE_PATH+"CPUforecast.xlsx";
		...
		for (Row row : sheet) {
			...
			try {
				for (int cn = 0; cn <= row.getLastCellNum(); cn++) {
					if (row.getRowNum() > 0 && row.getRowNum() <=24) {
		...
		System.out.println("CPUForecast loaded successfully");
	}
	...
	public static String getT2Date() {
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd ");
		Calendar cal = Calendar.getInstance();
