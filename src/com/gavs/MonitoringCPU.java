...
public class MonitoringCPU {
	...
	public static void uploadSolrData() throws IOException {
		...
		deleteSolrData();
		String fileName = GavelConstant.FILE_PATH+"MonitoringCPU.xlsx";
		...
		for (Row row : sheet) {
			...
			try {
				for (int cn = 0; cn <= row.getLastCellNum(); cn++) {
					...
					if (row.getRowNum() > 11 && row.getRowNum()<=35) {
					...
					if (row.getRowNum() > 35 && row.getRowNum() <=59 ) {
					...
					if (row.getRowNum() > 59 && row.getRowNum() <=83 ) {
						...
						if (row.getCell(cn) != null) {
							...
							if (cn == 14) {
								...
								String valueAsInExcel = fmt.formatCellValue(row.getCell(cn));
								if (!valueAsInExcel.isEmpty() && valueAsInExcel != null) {
									PollTime = getFourDate() + valueAsInExcel;
									master.addField("PollTime", getDate(PollTime));
								}
							}
							if (cn == 15) {
					...
					if (row.getRowNum() > 83 && row.getRowNum() <= 107 ) {
					...
					if (row.getRowNum() > 107 && row.getRowNum() <= 131 ) {
		...
		System.out.println("MonitoringCPU loaded successfully");
	}
