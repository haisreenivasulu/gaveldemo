...
public class MemoryForecastClosed {

	private static final String master = GavelConstant.SOLR_URL +""+GavelConstant.GAVEL_COMPANY +"-prediction-memoryforecast";
	private static String key = "";

	public static void uploadSolrData() throws IOException {
		SolrServer serverMaster = new HttpSolrServer(master);
		String fileName = GavelConstant.FILE_PATH+"MemoryforecastClosed.xlsx";
					...
					if (row.getRowNum() > 0) {
						...
						if (row.getCell(cn) != null) {
							...
							if (cn == 0) {
								...
								String gDate = getTodayDate() + valueAsInExcel;
								master.addField("PredictionDate", getDate(gDate));
		...
		System.out.println("Closed MemoryForecast loaded successfully");
	}
	...
	public static String getDate(String createdDate) {
		...
		try {
			...
			// System.out.println(createdDate +" "+new
			// SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'").format(new
			// Date(createDate.getTime()+1000)));
